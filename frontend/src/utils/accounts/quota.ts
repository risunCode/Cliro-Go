...
export type QuotaBucket = NonNullable<NonNullable<Account['quota']>['buckets']>[number]
export type QuotaDisplayStatus = 'ok' | 'low' | 'exhausted' | 'empty' | 'unknown'
...
export const isQuotaExhausted = (quota: Account['quota']): boolean => {
  if (!quota) {
    return false
  }
