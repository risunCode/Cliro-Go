...
func extractInlineCode(value string) string {
	...
	if index >= 0 {
		...
		for _, sepSym := range []string{":", "="} {
			if sep := strings.Index(segment, sepSym); sep >= 0 {
...
func stripLeadingAuthCode(message, code string) string {
	trimmed := strings.TrimSpace(message)
	if trimmed == "" {
		return ""
	}
...
func isKnownAuthCode(code string) bool {
	...
	return normalized != "" && (isBlockedAuthCode(normalized) || isRefreshableAuthCode(normalized))
}
